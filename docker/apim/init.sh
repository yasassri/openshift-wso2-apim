...
#Changing the loging location
POD_ID=$(hostname)
sed -i "s&XX_POD_ID_XX&${POD_ID}&g" ${WSO2_SERVER_HOME}/repository/conf/log4j.properties
sed -i "s&XX_POD_ID_XX&${POD_ID}&g" ${WSO2_SERVER_HOME}/bin/wso2server.sh
sed -i "s&XX_POD_ID_XX&${POD_ID}&g" ${WSO2_SERVER_HOME}/repository/conf/tomcat/catalina-server.xml
sed -i "s&XX_POD_ID_XX&${POD_ID}&g" ${WSO2_SERVER_HOME}/repository/conf/access-log.properties
